const StandardBounties = artifacts.require("../contracts/StandardBounties/StandardBounties");
const OceanToken = artifacts.require("../contracts/Ocean/OceanToken");
...
contract('StandardBounties', function(accounts) {
  ...
  before(async () => {
  	bountyToken = await OceanToken.new()
  	console.log('Deployed Bounty Token ==>', bountyToken.address)
  	bountyToken.initialize(accounts[0], accounts[0])
  	// await bountyToken.addMinter(accounts[0])
  	let isMinter0 = await bountyToken.isMinter.call(accounts[0])
  	console.log('Is minter?', accounts[0], isMinter0)
  	await bountyToken.mint(accounts[0], web3.utils.toWei('1000', 'ether'))
  })
  ...
  it("[ERC20] Verifies that I can issue a bounty paying in ERC20 tokens while locking up funds", async () => {
    ...
    const rs = await registry.issueAndContribute(accounts[0], [accounts[0]], [accounts[1], accounts[2]], "data", 2528821098, bountyToken.address, 20, 1, {from: accounts[0]});
